...
export function SimpleTradingPanel({
    ...
    return (
        <div className="bg-zinc-800/50 border border-zinc-700/50 rounded-xl p-3 md:p-4 space-y-3 md:space-y-4">
            {/* Current Price */}
            <div className="text-center pb-3 md:pb-4 border-b border-zinc-700">
            ...
            <div className="grid grid-cols-2 gap-3 md:gap-4 text-sm">
                <div>
                    ...
                    <div className="text-white font-mono text-sm md:text-base">${balance.toLocaleString()}</div>
                </div>
                <div>
                    <div className="text-zinc-500 text-xs">持有張數</div>
                    <div className="flex items-center gap-1">
                        <span className="text-white font-mono text-sm md:text-base">{position?.quantity || 0}</span>
                        {isShort && (
            ...
            {hasPosition && position && (
                <div className={`rounded-lg p-2.5 md:p-3 space-y-1.5 md:space-y-2 ${
            ...
            <div className="space-y-1.5 md:space-y-2">
                <div className="flex justify-between text-xs text-zinc-500">
                    ...
                    <span className="truncate ml-2">
                        {isShort 
                            ? `可回補: ${position?.quantity || 0}`
                            : isLong 
                                ? `可賣: ${position?.quantity || 0} / 可買: ${maxBuyLots}`
                                : `可買: ${maxBuyLots} / 可空: ${maxShortLots}`
                        }
                    </span>
                </div>
                <input
                    ...
                    className="w-full bg-zinc-900 border border-zinc-700 rounded-lg px-3 py-2 text-white font-mono text-sm md:text-base focus:outline-none focus:border-indigo-500"
                    min={1}
                />
                ...
                <div className="grid grid-cols-4 gap-1">
                    {[1, 5, 10, 50].map((q) => (
                        <button
                            ...
                            className={`py-1 md:py-1.5 rounded text-xs font-medium transition-all ${
                                quantity === q
            ...
            {/* Order Preview - Collapsible on mobile */}
            ...
            <div className="grid grid-cols-2 gap-2">
                <button
                    ...
                    className={`py-2.5 md:py-3 rounded-lg font-medium text-sm md:text-base transition-all disabled:opacity-50 disabled:cursor-not-allowed ${upBg} hover:opacity-90 text-white`}
                >
                    {isShort ? '回補' : '買入'}
                </button>
                <button
                    ...
                    className={`py-2.5 md:py-3 rounded-lg font-medium text-sm md:text-base transition-all disabled:opacity-50 disabled:cursor-not-allowed ${downBg} hover:opacity-90 text-white`}
                >
                    {isLong ? '賣出' : '做空'}
                </button>
            </div>
            ...
            {hasPosition && position && (
                <button
                    ...
                    className={`w-full py-2 rounded-lg border text-xs md:text-sm transition-all ${
                        isShort 
                ...
                >
                    {isShort 
                        ? `空頭平倉 (${position.quantity}張)`
                        : `多頭平倉 (${position.quantity}張)`
                    }
                </button>
            )}
            
            {/* Trade History - Hidden on very small screens when trading, collapsed display */}
            {trades.length > 0 && (
                <div className="pt-3 md:pt-4 border-t border-zinc-700">
